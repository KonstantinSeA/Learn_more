...
def index():
    ...
    if current_user.is_authenticated:
        news = db_sess.query(News).filter(
            (News.user == current_user) | (News.is_private == False)).all()
    else:
...
def cookie_test():
    ...
    if visits_count:
        ...
        ans.set_cookie('visits_count', str(visits_count + 1),
                       max_age=60 * 60 * 24)
    else:
        ans = make_response(f"Вы пришли на страницу в первый раз за сутки")
